...
% Boran Tae 
    ...
    {\color{blue}
		    	The idea behind our algorithm is to integrate tree-based predictors with contrastive pattern mining algorithms. Unlike traditional explainers for credit risk prediction, which are typically formulated as post-hoc explanation techniques based on feature attribution methods (e.g., SHAP or LIME), our algorithm maintains a separation between the prediction and explanation processes while adopting contrastive pattern mining as the explanatory mechanism. Furthermore, both the predictor and the explanation are grounded in tree-based structures, ensuring a structural consistency between the prediction and the generated explanations.\newline
